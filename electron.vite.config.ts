    ...
    return {
        main: {
            ...
            define: {
                ...
                'process.env.DEMO_MODE': JSON.stringify(env.DEMO_MODE || process.env.DEMO_MODE || ''),
            }
        },
        preload: {
            plugins: [externalizeDepsPlugin()]
            ...
            build: {
                rollupOptions: {
                    input: {
                        ...
                        // Duplicate local DB from a Cieloo instance (Dev menu, Full Local)
                        duplicateDb: resolve('src/renderer/duplicate-db.html'),
